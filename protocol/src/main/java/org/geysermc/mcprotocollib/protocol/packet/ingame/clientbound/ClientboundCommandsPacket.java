        ...
        for (int i = 0; i < this.nodes.length; i++) {
            ...
            if (type == CommandType.ARGUMENT) {
                ...
                switch (parser) {
                    ...
                    case RESOURCE_OR_TAG, RESOURCE_OR_TAG_KEY, RESOURCE, RESOURCE_KEY, RESOURCE_SELECTOR -> properties = new ResourceProperties(MinecraftTypes.readResourceLocationString(in));
                    default -> {
                    }
                }

                if ((flags & FLAG_SUGGESTION_TYPE) != 0) {
                    suggestionType = MinecraftTypes.readResourceLocationString(in);
                }
            }

...
import static org.geysermc.mcprotocollib.protocol.codec.MinecraftTypes.readTag;
import static org.geysermc.mcprotocollib.protocol.codec.MinecraftTypes.writeTag;
...
public class ItemTypes {
    ...
    public static ItemEnchantments readItemEnchantments(ByteBuf buf) {
        ...
        Int2IntMap enchantments = new Int2IntOpenHashMap(enchantmentCount);
    ...
    public static AdventureModePredicate.BlockPredicate readBlockPredicate(ByteBuf buf) {
        ...
        MNBT nbt = MinecraftTypes.readNullable(buf, MinecraftTypes::readMNBT);
    ...
    public static ArmorTrim.TrimMaterial readTrimMaterial(ByteBuf buf) {
        ...
        Map<String, String> assetOverrides = new HashMap<>();
        int overrideCount = MinecraftTypes.readVarInt(buf);
        for (int i = 0; i < overrideCount; i++) {
            assetOverrides.put(MinecraftTypes.readResourceLocationString(buf), MinecraftTypes.readString(buf));
        }
    ...
    public static void writeTrimMaterial(ByteBuf buf, ArmorTrim.TrimMaterial material) {
        ...
        for (Map.Entry<String, String> entry : material.assetOverrides().entrySet()) {
    ...
    public static <T extends Enum<?>> TypedEntityData<T> readTypedEntityData(ByteBuf buf, Function<ByteBuf, T> reader) {
        return new TypedEntityData<>(reader.apply(buf), MinecraftTypes.readMNBT(buf));
    }
    ...
    public static <T> void writeTypedEntityData(ByteBuf buf, TypedEntityData<T> typedEntityData, BiConsumer<ByteBuf, T> writer) {
        ...
        MinecraftTypes.writeMNBT(buf, typedEntityData.tag());
    }
